...
const coverPath = resolve(root, "production", "covers", "output", "i-am-what-happens-cover-wrap.pdf");
const coverInput = resolve(root, "dist", "books", "i-am-what-happens", "cover-print", "index.html");
const coverStylesheet = resolve(root, "public", "i-am-what-happens-cover.css");

const sheetWidthIn = 11.444444;
...
async function cssWithEmbeddedAssetUrls(css) {
  ...
  for (const match of matches) {
    ...
    const dataUrl = `url("data:${mimeTypeForAsset(absolutePath)};base64,${buffer.toString("base64")}")`;
    result = result.replace(fullMatch, dataUrl);
  }
...
async function writeCoverPdf(pngBuffer) {
  ...
  pdf.setTitle("I Am What Happens - Paperback Cover");
...
console.log("Building Astro cover route for I Am What Happens...");
