...
const coverPath = resolve(root, "production", "covers", "output", "the-subconscious-advantage-cover-wrap.pdf");
const coverInput = resolve(root, "dist", "books", "subconscious-advantage", "cover-print", "index.html");
const coverStylesheet = resolve(root, "public", "subconscious-cover.css");

const sheetWidthIn = 11.6666667;
const sheetHeightIn = 8.75;
const cssPixelsPerInch = 96;
const printDpi = 300;
const viewportWidth = Math.round(sheetWidthIn * cssPixelsPerInch);
const viewportHeight = Math.round(sheetHeightIn * cssPixelsPerInch);
...
async function renderCoverPng() {
  ...
  try {
    ...
    if (dimensions.width !== expectedRasterWidth || dimensions.height !== expectedRasterHeight) {
      throw new Error(
        `Expected ${expectedRasterWidth}x${expectedRasterHeight}px cover raster, got ${dimensions.width}x${dimensions.height}px.`
      );
    }
...
async function writeCoverPdf(pngBuffer) {
  ...
  pdf.setTitle("The Subconscious Advantage - Paperback Cover");
...
console.log("Building Astro cover route for The Subconscious Advantage...");
await run(nodeCommand, [astroCommand, "build"]);

console.log(`Rendering high-resolution ${expectedRasterWidth}x${expectedRasterHeight}px cover image...`);
