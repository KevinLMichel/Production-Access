...
const coverPath = resolve(root, "production", "covers", "output", "freedom-by-design-cover-wrap.pdf");
const coverInput = resolve(root, "dist", "books", "freedom-by-design", "cover-print", "index.html");
const coverStylesheet = resolve(root, "public", "freedom-cover.css");
...
console.log("Building Astro cover route for Freedom by Design...");
await run(nodeCommand, [astroCommand, "build"]);

console.log("Generating Freedom by Design KDP paperback cover PDF...");
await run(nodeCommand, [
  ...
  coverPath,
  "--title",
  "Freedom by Design - Paperback Cover",
  "--author",
  "Kevin L. Michel"
]);
