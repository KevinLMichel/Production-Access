...
const coverPath = resolve(root, "production", "covers", "output", "clock-with-no-hands-cover-wrap.pdf");
const coverInput = resolve(root, "dist", "books", "clock-with-no-hands", "cover-print", "index.html");
const coverStylesheet = resolve(root, "public", "clock-with-no-hands-cover.css");

const sheetWidthIn = 11.583333;
...
async function writeCoverPdf(pngBuffer) {
  ...
  pdf.setTitle("The Clock with No Hands - Paperback Cover");
...
console.log("Building Astro cover route for The Clock with No Hands...");
